...
class LabourAttendance(Document):	
	...
	# 		if(frappe.db.exists('Labour Attendance',{"posting_date":self.posting_date,"project":self.project,'attendance_type':self.attendance_type,'subcontractor':self.subcontractor,'docstatus':1})):
	...
	def duplicate_validate(self):
		...
		if(self.attendance_type == 'Subcontractor'):
			exist_doc=frappe.get_list("Labour Attendance",filters={"posting_date":self.posting_date,"project":self.project,"attendance_type":self.attendance_type,"subcontractor":self.subcontractor,"docstatus":1})
			if(exist_doc):
				frappe.throw("This Subcontractor Attendance Already Present")
